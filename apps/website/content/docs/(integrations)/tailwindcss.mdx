...
<Callout> Remember that CVB works with any CSS approach, not just Tailwind. You can mix Tailwind with CSS Modules, CSS-in-JS, or any other styling method while still getting the benefits of CVB's variant system. </Callout>
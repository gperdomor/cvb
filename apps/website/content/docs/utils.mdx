...
<Callout type="info">
  The `cx` function only accepts string expressions as arguments, preferring a more functional approach over object syntax.
  This design choice leads to a more intuitive syntax with conditions first and better support for conditional branches through
  ternary operators.
</Callout>
...
<Callout type="info">
  See our Tailwind CSS integration [guide](./tailwindcss#handling-style-conflicts) for details.
</Callout>
